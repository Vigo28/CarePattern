...
def process_datapoints(datapoints, output_path: Path):
    if datapoints[0].keypoints is not None and datapoints[0].boxes is not None and datapoints[0].boxes.id is not None:
        ...
        for person_keypoints, tid in zip(datapoints[0].keypoints.data, datapoints[0].boxes.id):
            ...
            if hands_up and not hands_currently_up.get(tid, False):
                ...
                # Save hand up to file
                with open(output_path, 'a') as f:
                    f.write(f'Person {tid} | Hands above head, count:{hands_above_head_counts[tid]}\n')
